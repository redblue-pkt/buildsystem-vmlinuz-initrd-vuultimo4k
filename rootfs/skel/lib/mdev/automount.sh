...
mk_partition () {
	...
	mk_standard_part () {
		sgdisk -a 1 -n 1:2048:34815 -c 1:"kernel" /dev/mmcblk0
		sgdisk -a 1 -n 2:34816:67583 -c 2:"initrd" /dev/mmcblk0
		sgdisk -a 1 -n 3:67584:71679 -c 3:"splash" /dev/mmcblk0
		sgdisk -a 1 -n 4:71680:${ENDSECTOR} -c 4:"rootfs" /dev/mmcblk0
		mkfs.ext4 $1
	}
	mk_multiboot_simple_part () {
		sgdisk -a 1 -n 1:2048:2175 -c 1:"startup" /dev/mmcblk0
		sgdisk -a 1 -n 2:2176:34943 -c 2:"initrd" /dev/mmcblk0
		sgdisk -a 1 -n 3:34944:38911 -c 3:"splash" /dev/mmcblk0
		mkfs.vfat $1
	}
	mk_multiboot_part_1 () {
...
update_rootfs () {
	...
	|| [ -e ${destdir}/$1/$V_ROOTFS_4_FILENAME_GZ ]; then
		do_first_part () {
			...
			if [ $1 == 0 ]; then
				echo "boot emmcflash0.kernel_1 'root=/dev/mmcblk0p5 rootfstype=ext4 rootflags=data=journal rootwait rw coherent_pool=2M vmalloc=622m bmem=630m@394m bmem=383m@1665m bmem=443m@2625m'" >> /mnt/startup/STARTUP # boot from first partition default
				echo "boot emmcflash0.kernel_1 'root=/dev/mmcblk0p5 rootfstype=ext4 rootflags=data=journal rootwait rw coherent_pool=2M vmalloc=622m bmem=630m@394m bmem=383m@1665m bmem=443m@2625m'" >> /mnt/startup/STARTUP_1
				echo "boot emmcflash0.kernel_2 'root=/dev/mmcblk0p7 rootfstype=ext4 rootflags=data=journal rootwait rw coherent_pool=2M vmalloc=622m bmem=630m@394m bmem=383m@1665m bmem=443m@2625m'" >> /mnt/startup/STARTUP_2
				echo "boot emmcflash0.kernel_3 'root=/dev/mmcblk0p9 rootfstype=ext4 rootflags=data=journal rootwait rw coherent_pool=2M vmalloc=622m bmem=630m@394m bmem=383m@1665m bmem=443m@2625m'" >> /mnt/startup/STARTUP_3
				echo "boot emmcflash0.kernel_4 'root=/dev/mmcblk0p11 rootfstype=ext4 rootflags=data=journal rootwait rw coherent_pool=2M vmalloc=622m bmem=630m@394m bmem=383m@1665m bmem=443m@2625m'" >> /mnt/startup/STARTUP_4
				echo "boot -fatfs usbdisk0:zImage 'root=/dev/sdb2 rw rootwait rootflags=data=journal rootfstype=ext4 systemd.gpt_auto=0 consoleblank=0 vt.global_cursor_default=0 loglevel=7 coherent_pool=2M vmalloc=622m bmem=630m@394m bmem=383m@1665m bmem=443m@2625m'" >> /mnt/startup/STARTUP_USB_HDD
				echo "boot -fatfs usbdisk0:zImage 'root=/dev/sda2 rw rootwait rootflags=data=journal rootfstype=ext4 systemd.gpt_auto=0 consoleblank=0 vt.global_cursor_default=0 loglevel=7 coherent_pool=2M vmalloc=622m bmem=630m@394m bmem=383m@1665m bmem=443m@2625m'" >> /mnt/startup/STARTUP_USB_NOHDD
			elif [ $1 == 1 ]; then
